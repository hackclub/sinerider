...
void main(void) {
    ...
    uv.x += .5-.5*resolution.x/resolution.y;

    vec3 col = vec3(0.);
    
    vec2 sunPos = vec2(0.5, 0.4 - 0.7 * iTime/5.);
...
/*    col = mix(
  ...
  */  
    ...
    // fade to black
    col = mix(col, vec3(0.050980, 0.058823, 0.117647), smoothstep(0., 12., iTime));

    vec2 sampleUv = gl_FragCoord.xy/resolution.xy;

    vec3 starCol = texture2D(stars, sampleUv).rgb;
    col = mix(col, starCol, (1.0 - length(col)) * smoothstep(0., 12., iTime));
