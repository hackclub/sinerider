...
function GLUtils(gl) {
  ...
  function Program({vert, frag}) {
      ...
      self.instancedAttributes = (ext, array, layout) => {
          ...
          for ({ type, name, perInstance, stride = null, offset = null } of layout) {
              ...
              gl.vertexAttribPointer(location, length, gl.FLOAT, false, 
                  (stride || 0) * FLOAT_SIZE, (offset || 0) * FLOAT_SIZE)
      ...
      self.attrib = (name, length, stride, offset) => {
          ...
          gl.enableVertexAttribArray(location)
          gl.vertexAttribPointer(location, length, gl.FLOAT, false, 
              stride * FLOAT_SIZE, offset * FLOAT_SIZE)
      ...
      self.uniform = (name, value) => {
          ...
          else throw `Unexpected length ${value.length} when invoking GLUtils.Program.uniform() (${value})`
  ...
  function createReadBuffer(width, height, format) {
      const formatSize = format == gl.RGB 
          ? 3
          : format == gl.RGBA 
              ? 4 
              : null;
  ...
  function Texture(size = null, format = null) {
      ...
      if (size) {
          if (!format) throw `Expected size and format when constructing GLUtils.Texture`
          gl.texImage2D(
              gl.TEXTURE_2D,
              0,
              format,
  ...
  function Vertices(drawMode, attributes) {
      ...
      const buffer = _.flattenDeep(_.zip(...values.map(attr => _.chunk(attr.data, lengths[attr.type]))))
  ...
  function InstancingExtension() {
      ...
      if (!ext) {
          alert('WebGL feature \'ANGLE_INSTANCED_arrays\' must be enabled in order to run this webpage. Try switching or updating your browser!')
      ...
      const drawArraysInstanced = (mode, first, count, primcount) => ext.drawArraysInstancedANGLE(mode, first, count, primcount)

      const vertexAttribDivisor = (location, divisor) => ext.vertexAttribDivisorANGLE(location, divisor)
